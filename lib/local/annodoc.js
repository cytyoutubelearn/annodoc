...
//         		      function(a, b) { return b.replace(/\s+/g, ' '); });
    ...
    var parseAnnNormalization = function(line, log) {
        ...
        if (!m) {
            log('failed to parse normalization: "'+line+'"');
            return null;
        }
        ...
        if (!m) {
            log('failed to parse normalization id "'+norm_id+'": "'+line+'"');
            return null;
        }
        ...
        return [id, type, target, refdb, refid, text];
    };
    ...
    var displaySpanInformation = function(evt, target, spanId, spanType, 
        ...
        $.each(normalizations, function(normNo, norm) {
            var dbName = norm[0], dbKey = norm[1], normText = norm[2];
            // brat style, with id floated right
            // comment += ( '<hr/>' +
            ...
            //                 Util.escapeHTML(normText)+'</span>');
            // }
            // simplified style with everything in one span
            if(normNo==0) {
                comment += '<hr style="margin:10px"/>';
            } else {
                comment += '<br/>';
            }
            comment += ('<span class="norm_info_value">'+
                        Util.escapeHTML(normText) + ' (' +
                        Util.escapeHTML(dbName) + ':' +
                        Util.escapeHTML(dbKey) + ')</span>');
        });
